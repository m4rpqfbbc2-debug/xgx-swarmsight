            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation done, but the entire section may not be complete yet) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When it is the final answer, show a special prompt -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - match prediction fact count format
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - full extraction, no limit on count
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible with old format: single platform marker only)
      ...
      // Extract key quotes (compatible with multiple quote mark formats)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Prefer Chinese question numbering format first
      const cnPattern = /(?:^|[\r\n]+)Question\s*(\d+)[：:]\s*/g
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 heading (## xxx), since the section title is already displayed in the outer layer
  ...
  // Clean <br> before paragraph opening tags after block-level elements
...
.section-number {
  ...
  font-size: 16px;
  color: #78716c; /* Dark gray, does not change with status */
  font-weight: 500;
}
...
.right-panel {
  ...
  --wf-active-bg: #110f0d;
  --wf-active-border: #292524;
  --wf-active-dot: #292524;
  --wf-active-text: #292524;

  --wf-done-bg: #110f0d;
  --wf-done-border: #292524;
  --wf-done-dot: #c2410c;
