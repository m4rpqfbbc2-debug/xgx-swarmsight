...
# Add filter at module load time to ensure it takes effect before camel code executes
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable OASIS library verbose logging output
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last read maximum rowid value (using rowid instead of created_at because different platforms use different created_at formats)
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise falls back to standard)
...
async def main():
    ...
    # Create shutdown event at the beginning of main to ensure the entire program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        # Do not sys.exit() directly; let the asyncio loop exit normally and clean up resources
