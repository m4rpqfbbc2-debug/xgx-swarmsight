...
logger = get_logger('swarmsight.zep_tools')
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (truncate at sentence boundary, not hard truncation)
...
class ZepToolsService:
    ...
    - get_all_edges - Get all edges of the graph (with temporal info)
    ...
    def insight_forge(
        ...
        # Get details for all relevant entities (no quantity limit, full output)
        entity_insights = []
    ...
    def _generate_sub_queries(
        ...
        Decompose a complex question into multiple independently retrievable sub-queries
        """
...
Return the sub-question list in JSON format."""
    ...
    def interview_agents(
        ...
        # Step 4: Call real interview API (no platform specified, defaults to both platforms)
        try:
...
1. Distill key viewpoints from each party
