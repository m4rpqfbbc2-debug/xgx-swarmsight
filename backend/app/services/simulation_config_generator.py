...
logger = get_logger('swarmsight.simulation_config')
...
- The user base is Chinese, so it should follow Beijing Time daily routines
...
- After 11 PM: activity declining (activity coefficient 0.5)
- General pattern: low activity at dawn, gradually increasing in morning, moderate during work hours, peak in evening
- **Important**: The example values below are for reference only. You need to adjust specific time periods based on the nature of events and participant group characteristics.
...
{{
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent config using rules (Chinese daily routine)"""
