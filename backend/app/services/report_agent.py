...
2. Plan outline structure first, then generate in sections
3. Each section uses ReACT multi-round thinking and reflection pattern
4. Supports conversation with users, autonomously calling retrieval tools during dialog
"""
...
logger = get_logger('swarmsight.report_agent')
...
class ReportLogger:
    ...
    def log(
        ...
        Args:
            ...
            details: Detail dictionary, no truncation
    ...
    def log_section_content(
        ...
        """Record section content generation complete (content only, does not represent entire section completion)"""
        self.log(
...
class ReportConsoleLogger:
    ...
    These logs differ from agent_log.jsonl - they are plain text format console output.
    """
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'swarmsight.report_agent',
            'swarmsight.zep_tools',
        ]
...
3. [Language Consistency - Quoted Content Must Be Translated to the Report Language]
   ...
   - The report language should match the language of the simulation requirement and source materials
   - When quoting content in a different language, translate it fluently into the report language before including it
...
Social media served as the front line for information dissemination:
...
Each reply can only do one of the following two things (not both at the same time):
...
4. [Quote Format Specification - Must Be Standalone Paragraphs]
   ...
   > "The response approach appeared rigid and slow in the fast-changing social media environment."
   ...
   Incorrect format:
   ```
   The response was considered lacking in substance. > "The response approach..." This evaluation reflects...
   ```
...
REACT_UNUSED_TOOLS_HINT = "\nHint: You have not yet used: {unused_list}. Consider trying different tools for multi-angle information."
...
class ReportAgent:
    ...
    2. Generation stage: Generate content section by section, each section can call tools multiple times for information
    ...
    # Valid tool names set for bare JSON fallback parsing validation
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try when format 1 did not match, avoid false matching JSON in body text
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflicts of tool call + Final Answer appearing together
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Since section titles are system-added, content should not have any headings
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (same content heading appearing within 5 consecutive lines)
                is_duplicate = False
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Convert other h1 headings to bold
                        processed_lines.append(f"**{title}**")
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Convert non-section h2 headings to bold
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
