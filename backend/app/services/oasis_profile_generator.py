...
logger = get_logger('swarmsight.oasis_profile')
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graphs into Agent Profiles required by OASIS simulation

    Features:
    1. Calls Zep graph retrieval functions for richer context
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        separately and merge the results. Uses parallel requests for efficiency.
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, maximizing fidelity to existing real-world situations. You must return valid JSON format, and all string values must not contain unescaped newlines. Use English for all output."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximizing fidelity to existing real-world situations.
...
2. persona: Detailed persona description (2000 characters of plain text), should include:
   ...
   - Personal memory (important part of the persona, introduce the individual's connection to events, and their existing actions and reactions in events)
3. age: Age number (must be an integer)
4. gender: Gender, must be in English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP, etc.)
6. country: Country (in English, e.g., "US", "UK", "China")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this organization/group entity, maximizing fidelity to existing real-world situations.
...
2. persona: Detailed account profile description (2000 characters of plain text), should include:
   ...
   - Organization memory (important part of the persona, introduce the organization's connection to events, and its existing actions and reactions in events)
...
- Organization account speech must align with its identity positioning"""
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Chinese to English mapping (keys are Chinese gender terms from LLM output)
        gender_map = {
            "\u7537": "male",  # Chinese: male
