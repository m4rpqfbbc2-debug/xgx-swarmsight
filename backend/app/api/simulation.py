...
logger = get_logger('swarmsight.api.simulation')
...
def create_simulation():
    ...
    Request (JSON):
        {
            ...
            "graph_id": "swarmsight_xxxx",    // Optional, fetched from project if not provided
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Knowledge graph has not been built yet. Please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True, it's already done
...
def prepare_simulation():
    ...
    try:
        ...
        # This allows frontend to immediately get the expected total Agent count after calling prepare
        try:
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: Time configuration (simulation duration, rounds, peak/valley periods)
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "swarmsight_xxxx",     // Required
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} preparation is complete, resetting status to ready (original status: {state.status.value})")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires a valid graph_id. Please ensure the project has a built knowledge graph"
                }), 400
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entered wait-for-command mode after completing simulation rounds)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been shut down. Please ensure the simulation has completed and entered wait-for-command mode."
            }), 400
...
def close_simulation_env():
    ...
    Sends a close environment command to the simulation, making it gracefully exit wait-for-command mode.

    Note: This differs from /stop. /stop forcefully terminates the process,
    while this endpoint gracefully closes the environment.
